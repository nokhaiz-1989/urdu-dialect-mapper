...
# Plot dialect regions
for dialect in data["Dialect Cluster"].dropna().unique():
    subset = data[data["Dialect Cluster"] == dialect]
    color = assign_color(dialect)
    if selected_dialect_map == "All" or dialect == selected_dialect_map:
        polygon = folium.FeatureGroup(name=dialect)
